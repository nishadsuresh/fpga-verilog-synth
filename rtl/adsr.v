...
module adsr #(
    ...
    wire [PRODUCT_WIDTH-1:0] attack_product  = {{COUNTER_WIDTH{1'b0}}, ENV_MAX} * counter;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (gate && !gate_prev) begin
                // rising edge: (re)start attack from wherever we are now
                state <= ST_ATTACK;
                counter <= 0;
                ...
                case (state)
                    ...
                    ST_ATTACK: begin
                        if (attack_samples == 0 || counter >= attack_samples) begin
                            ...
                            envelope <= attack_product / attack_samples;
                            counter <= counter + 1;
                        end
                    end
